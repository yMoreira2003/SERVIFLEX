...
{
    public class LogListarServiciosPorCategoriaId
    {
        public ResListarServiciosPorCategoriaId listarServicios(ReqListarServiciosPorCategoriaId req)
        {
            ResListarServiciosPorCategoriaId res = new ResListarServiciosPorCategoriaId();
            ...
            try
            {
                ...
                if (req.Categoria.CategoriaId <= 0)
                {
                    res.resultado = false;
                    res.error.Add(new Error
                    {
                        ErrorCode = (int)enumErrores.idFaltante,
                        Message = "Categoria ID inválido"
                    });
                    return res;
                }
                ...
                List<SP_ListarServiciosPorCategoriaIdResult> resultado;

                using (LinQDataContext db = new LinQDataContext())
                {
                    resultado = db.SP_ListarServiciosPorCategoriaId(
                        req.Categoria.CategoriaId,
                        ref idReturn,
                        ref errorID,
        ...
        private List<Servicio> factoriaListaServicios(List<SP_ListarServiciosPorCategoriaIdResult> lista)
        {
